...
| **Best Model** | Tuned Random Forest — **95.4% Accuracy**, **0.9273 Macro F1**, **0.9262 Kappa** |
...
├── best_rf_model.joblib                    ← Saved Random Forest (tuned)
├── best_xgb_model.joblib                   ← Saved XGBoost (tuned)
...
> **Class Imbalance:** The dataset is severely imbalanced — Lodgepole Pine (48.8%) outnumbers Cottonwood/Willow (0.5%) by **103.1×**. This is addressed via `class_weight='balanced'` and evaluated with Macro F1.
...
- **Scaling:** `StandardScaler` applied to the 10 continuous features only; binary features left as-is
- **Class weights:** Computed via `compute_class_weight('balanced')` — Class 4 receives weight 30.215
- **Train/test split:** Stratified 80/20 split → Train: 464,809 samples, Test: 116,203 samples

### 4. Model Training
Three models trained with increasing complexity:
1. **Logistic Regression** — `max_iter=1000`, `class_weight='balanced'`
2. **Random Forest** — `n_estimators=200`, `class_weight='balanced'`
3. **XGBoost** — `n_estimators=200`, `objective='multi:softmax'`, `num_class=7`
...
### 7. Model Saving
Best tuned models and scaler persisted with `joblib`:
- `best_rf_model.joblib` — Tuned Random Forest
- `best_xgb_model.joblib` — Tuned XGBoost
...
- **Result:** 59.9% Accuracy, 0.5068 Macro F1 — struggles with non-linear decision boundaries
...
- **Result:** 95.5% Accuracy, 0.9257 Macro F1 — strong out-of-the-box performance

### XGBoost
- **Why:** Gradient-boosted trees; typically the strongest performer on tabular data
- **Parameters:** `n_estimators=200`, `objective='multi:softmax'`, `num_class=7`, `eval_metric='mlogloss'`
- **Result:** 90.7% Accuracy, 0.8937 Macro F1 — good but slightly below Random Forest on this dataset
...
|---|---|---|---|---|
| Logistic Regression | 0.5989 | 0.5068 | 0.6277 | 0.4301 |
| Random Forest | 0.9550 | 0.9257 | 0.9548 | 0.9274 |
| XGBoost | 0.9072 | 0.8937 | 0.9070 | 0.8504 |
| **Random Forest (Tuned)** | 0.9541 | **0.9273** | 0.9540 | **0.9262** |
| **XGBoost (Tuned)** | **0.9187** | 0.9071 | **0.9185** | 0.8690 |
...
- **Best Model:** Tuned Random Forest — **0.9273 Macro F1**, **95.4% Accuracy** on the test set
...
- **Impact of Tuning:**
  - Random Forest: Macro F1 improved 0.9257 → 0.9273 (+0.0016)
  - XGBoost: Macro F1 improved 0.8937 → 0.9071 (+0.0134) — tuning had a larger effect on XGBoost
...
*Macro F1 score comparison across all 5 model variants. Tuned Random Forest achieves the highest score (0.9273), with Tuned XGBoost as a close second (0.9071).*
...
6. Saved models (`best_rf_model.joblib`, `best_xgb_model.joblib`, `scaler.joblib`) are generated during Section 7
