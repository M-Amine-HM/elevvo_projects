...
# --------------------------------------------------------------------------
# API client config — the Gradio app is a thin HTTP client now.
# --------------------------------------------------------------------------
# Inside docker-compose the service is reachable by name: http://api:8000.
# When running the frontend on the host instead, point this at the API, e.g.
...
# --------------------------------------------------------------------------
# Defaults and scenario presets (unchanged from Task 9)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# API calling — the ONLY place the UI talks to the model (via HTTP).
# --------------------------------------------------------------------------
def call_predict_api(payload):
    """POST the six raw sensor readings to the FastAPI service.

    Returns (data, error_message). Exactly one of the two is truthy.
    """
    try:
        response = requests.post(PREDICT_URL, json=payload, timeout=REQUEST_TIMEOUT_S)
    except requests.exceptions.RequestException as exc:
        return None, (
            f"Cannot reach the prediction API at {PREDICT_URL}. "
            f"Start it with `docker-compose up --build`. ({type(exc).__name__})"
        )
    ...
    if response.status_code != 200:
        ...
        try:
            ...
            pass
        return None, f"API rejected the request (HTTP {response.status_code}): {detail}"
...
# --------------------------------------------------------------------------
# Result rendering (unchanged from Task 9 — pure display logic)
# --------------------------------------------------------------------------
...
def build_indicators(air, process, speed, torque, wear, ptype):
    delta = process - air
    power = torque * speed * 2 * math.pi / 60.0
    load = wear * torque
...
def render_api_error(message):
    ...
    <div class="result bad">
      <div class="verdict">
        <div class="verdict-text">
          <span class="badge bad"><i></i>API error</span>
          <p class="detail">{html.escape(message)}</p>
          <p class="action">Make sure the FastAPI service is up, then adjust a sensor reading to retry.</p>
        </div>
      </div>
    </div>"""
...
def predict_failure(air_temperature, process_temperature, rotational_speed, torque, tool_wear, product_type):
    ...
    data, error = call_predict_api(payload)
    if error:
        return render_api_error(error), {"error": error}

    # Rebuild Task 9's rendering inputs from the API response (no model client-side).
    probabilities_dict = data["failure_probabilities"]
    class_names = list(probabilities_dict.keys())
    probabilities = [float(p) for p in probabilities_dict.values()]
    try:
        no_failure_index = class_names.index("No failure")
    except ValueError:
        return render_api_error("API response is missing the No failure class."), {}
    ...
    result = {
        ...
        "failure_probabilities": probabilities_dict,
    }
...
with gr.Blocks(css=css, title="Machine failure risk | Predictive maintenance") as demo:
    ...
    <div class="app-header">
      <div>
        ...
        <p>Adjust the sensor readings and the assessment updates live — served by the Task 10 API.</p>
      </div>
    ...
    with gr.Row(equal_height=False, elem_id="main-row"):
        ...
        with gr.Column(scale=6, min_width=300, elem_classes="panel"):
            ...
            with gr.Accordion("Raw model output", open=False, elem_classes="raw-acc"):
                raw_output = gr.JSON(
                    label="Predicted failure type / risk / confidence / per-class probabilities")
